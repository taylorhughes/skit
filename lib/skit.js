...
function renderError(res, e, opt_message, opt_code) {
  var message = opt_message || 'Error processing request';
  res.writeHead(opt_code || 502, {'Content-Type': 'text/html; charset=utf-8'});
  res.write('<h1>' + escapeHtml(message) + '</h1>');
  if (e) {
    res.write('<h2 style="font-family: consolas, fixed-width">' + escapeHtml('' + e) + '</h2>');
    if (e) {
      res.write('<div style="padding: 20px; margin-top: 20px; background: #eee;"><pre>' + escapeHtml(e.stack) + '</pre></div>');
    }
  }
  res.end();
}
...
function loadResourcesForModule(module) {
  ...
  for (var i = 0; i < allResources.length; i++) {
    ...
    var object;
    try {
      object = eval(resource.getFunctionString()).apply({}, evaluatedDependencies);
    } catch(e) {
