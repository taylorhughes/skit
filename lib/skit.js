...
function renderControllerForRequest(root, req, originalUrl, res, shouldNavigate, shouldError, shouldFinish) {
  ...
  if (!controllerModule) {
    shouldError(404, {'message': 'Could not find any Controller at this path. Are you using Controller.create({}) to define your controller?'});
    return;
  }
  ...
  var renderFinal = function(controllerArgs) {
    try {
      var title = controller.__title__();
      var meta = controller.__meta__();
      var body = controller.__body__();
    } catch (e) {
      shouldError(500, {'message': 'Error rendering content: ' + e, 'stack': e.stack});
      return;
    }
  ...
  var loadNext = function() {
    ...
    console.log('... preloading ' + (controllersToLoadInOrder.length ? 'parent' : 'controller'));

    preload.call(controller, function(var_args) {
      if (navigation.__notfound__()) {
        shouldError(404, {'message': 'Resource not found at this path.'});
        return;
      }
      ...
      var load = CurrentControllerKlass.prototype.__load__ || function defaultLoad() {};
      load.apply(controller, controllerArgs);

      if (!controllersToLoadInOrder.length) {
        renderFinal(controllerArgs);
      } else {
        loadNext();
      }
    });
...
function serveStatic(req, res) {
  ...
  var sent = send(req, pathname, {root: path.resolve(PACKAGE_PATH, 'static')});
  sent.on('error', error)
  sent.pipe(res);
}
...
function collectDependenciesForPublicRoutes(packagePath) {
  ...
  console.log()
  console.log(allStylesheets.join('\n'))
  console.log()
}
...
function run(packagePath, opt_options) {
  ...
  var staticPath = path.resolve(PACKAGE_PATH, 'static');
  var useStatic = false;
  try {
...
module.exports = {
  ...
  'collectDependenciesForPublicRoutes': collectDependenciesForPublicRoutes,
};
