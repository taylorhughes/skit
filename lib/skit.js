...
function load(packagePath, _callback) {
...
function run(packagePath, opt_port) {
  ...
  var server = http.createServer(function(req, res) {
    load(packagePath, function(err, publicRoot, publicLoadedFiles) {
      ...
      controllerModule.getMainObject(function(err, ControllerKlass) {
        var controller = null;
        ...
        controller.__preload__(function(var_args) {
          ...
          res.write(
            ['<script>(function() {',
              'var ControllerKlass = window.skit.files[' + JSON.stringify(controllerModule.dependencyPath + ':js') + '];',
