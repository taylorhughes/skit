...
var transformers = require('./loader/transformers');
...
function run(packagePath, opt_options) {
  ...
  if (!options.debug) {
    // Start loading modules ASAP if we're in prodland, and cache the result.
      ...
      controllerModule.getMainObject(function(err, ControllerKlass, ControllerKlassPath) {
        ...
        controller.__preload__(function(var_args) {
          ...
          publicLoadedFiles.forEach(function(file) {
            ...
            var functionString = file.transformer.functionStringWithDependencies(localDeps, TargetEnvironment.BROWSER);
