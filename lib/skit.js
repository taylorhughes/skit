...
function renderControllerForRequest(req, originalUrl, res, shouldNavigate, shouldError, shouldFinish) {
  ...
  if (!controllerModule) {
    shouldError(404, 'Could not find any modules at this path.');
    return;
  }
  ...
  if (!ControllerKlass || !ControllerKlass.__controller__) {
    shouldError(404, route.modulePath + ' is not a controller. Use Controller.create({}) to define your controller.');
    return;
  }
      ...
      netproxy.__register__(name, {
        modifyRequest: function(apiRequest_) {
          ...
          try {
            ...
            shouldError(502, 'Proxy modifyRequest error.', e);
          }
        },
        modifyResponse: function(apiResponse) {
          try {
            ...
            shouldError(502, 'Proxy modifyResponse error.', e);
          }
        }
      });
    });
  ...
  var net = controllerResources.objectsByModulePath[NET_SERVER_MODULE];
  if (net) {
    net.__setErrorHandler__(function(e) {
      shouldError(502, 'Net error.', e);
    });
  }
  ...
  var loadNext = function() {
    ...
    if (navigation) {
      navigation.__reset__(req.url, req.headers['user-agent']);
    }
    ...
    try {
      preload.call(controller, preloadComplete);
    } catch (e) {
      shouldError(502, 'Error during preload.', e);
    }
  };
  loadNext();
}
