...
function ControllerRenderer(moduleLoader, bundles, server, request) {
...
ControllerRenderer.prototype.setupProxies_ = function() {
  var netproxy = this.moduleLoader.objectAtModulePath(PROXY_MODULE);
  if (!netproxy) {
    return;
  }
...
ControllerRenderer.prototype.setupNet_ = function() {
  var net = this.moduleLoader.objectAtResourcePath(NET_SERVER_MODULE);
  if (!net) {
    return;
  }
...
ControllerRenderer.prototype.setupCookies_ = function() {
  var cookies = this.moduleLoader.objectAtResourcePath(COOKIES_SERVER_MODULE);
  if (!cookies) {
    return;
  }
...
ControllerRenderer.prototype.serve = function() {
  var ControllerKlass = this.moduleLoader.mainObject();
...
ControllerRenderer.prototype.preloadController = function(ControllerKlass, onComplete) {
  ...
  var navigation = this.moduleLoader.objectAtResourcePath(NAVIGATION_MODULE);
  if (navigation) {
...
ControllerRenderer.prototype.render = function(controllerProperties) {
  ...
  this.bundles.forEach(function(bundle) {
    bundle.styles.forEach(function(resource) {
      var cssUrl = this.server.getResourceUrl(resource);
      if (!(cssUrl in cssUrls)) {
        cssUrls[cssUrl] = 1;
        cssBundleUrls.push({bundle: bundle.name, url: cssUrl});
      }
    }, this);

    bundle.scripts.forEach(function(resource) {
      if (resource.includeInEnvironment(TargetEnvironment.BROWSER)) {
        var jsUrl = this.server.getResourceUrl(resource);
        if (!(jsUrl in jsUrls)) {
          jsUrls[jsUrl] = 1;
          jsBundleUrls.push({bundle: bundle.name, url: jsUrl});
        }
      }
    }, this);
  }, this);
  ...
  var html = BOOTSTRAP_TEMPLATE({
    ...
    controllerModulePath: this.moduleLoader.mainObjectResourcePath(),
