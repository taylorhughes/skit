...
function ControllerRenderer(server, controllerModule, request) {
  ...
  this.cssResources = server.globalCssResources().concat(resources.cssResources);
  this.scriptResources = resources.scriptResources;
  this.objectsByModulePath = resources.objectsByModulePath;
...
ControllerRenderer.prototype.setupProxies_ = function() {
  var netproxy = this.objectsByModulePath[PROXY_MODULE];
  if (!netproxy) {
    return;
  }
...
ControllerRenderer.prototype.setupNet_ = function() {
  var net = this.objectsByModulePath[NET_SERVER_MODULE];
  if (!net) {
    return;
  }
...
ControllerRenderer.prototype.setupCookies_ = function() {
  var cookies = this.objectsByModulePath[COOKIES_SERVER_MODULE];
  if (!cookies) {
    return;
  }
...
ControllerRenderer.prototype.serve = function() {
  var ControllerKlass = this.objectsByModulePath[this.module.modulePath];
...
ControllerRenderer.prototype.preloadController = function(ControllerKlass, onComplete) {
  ...
  var navigation = this.objectsByModulePath[NAVIGATION_MODULE];
  if (navigation) {
...
ControllerRenderer.prototype.render = function(controllerProperties) {
  ...
  var cssUrls = this.cssResources.map(function(resource) {
    return this.server.getResourceUrl(resource);
  }, this);
  cssUrls = skitutil.unique(cssUrls);

  var scripts = this.scriptResources;
  ...
  if (PROXY_RESOURCE in this.objectsByModulePath) {
    this.server.eachProxy(function(name, proxy) {
