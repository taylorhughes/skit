...
function SkitServer(packagePath, opt_options) {
  ...
  // The base for finding public controller modules.
  this.publicRoot = options.publicRoot || 'public';

  // Optional environment variables sent to client, accessible from JS.
  this.env = {};
  if (options.env) {
    for (var k in options.env) {
      this.env[k] = options.env[k];
    }
  }
  this.env['publicRoot'] = this.publicRoot;
  if (this.debug) {
    this.env['debug'] = true;
  }

  var bundleConfiguration = options.bundleConfiguration || [{name: 'all', paths: ['/*']}];
  this.loader = new BundledLoader(this.packagePath, this.publicRoot, bundleConfiguration);
...
SkitServer.prototype.renderError = function(req, res, e) {
  ...
  if (this.renderCustomError(status, req, res, e)) {
    return;
  }
...
SkitServer.prototype.serveControllerFromRequest = function(req, res) {
  ...
  var requestInfo = moduleInfo.requestInfo;
  if (this.redirectWithTrailingSlashes && requestInfo.path.lastIndexOf('/') != requestInfo.path.length - 1) {
    var slashedUrl = req.url.replace(requestInfo.path, requestInfo.path + '/');
    this.renderRedirect(req, res, slashedUrl, true);
    return;
  }

  this.serveController(moduleInfo.module, requestInfo, req, res);
};
...
SkitServer.prototype.serveControllerScoped_ = function(scope, requestInfo, req, res, opt_code) {
  ...
  controllerRenderer.on(ControllerRenderer.REDIRECT, (function(redirectUrl, permanent) {
    done();
    this.renderRedirect(req, res, redirectUrl, permanent);
  }).bind(this));
