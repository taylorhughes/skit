...
SkitServer.prototype.serveController = function(req, res) {
  ...
  var loadNext = function() {
    try {
      ...
      pooledmoduleloader.borrowModuleScope(CONTROLLER_POOL, module, function(scope) {
        try {
          loadNextInner_(scope, moduleInfo);
        } catch (e) {
          scope.release();
  ...
  var loadNextInner_ = function(scope, moduleInfo) {
    var bundles = self.loader.bundlesRequiredForModule(scope.module);

    var controllerRenderer = new ControllerRenderer(scope, bundles, self, {
      url: req.url,
