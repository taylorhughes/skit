...
SkitModule.prototype.getObjectNamed = function(name, callback) {
  this.load_(function(err) {
    ...
    console.log(this.toString(), 'loaded', name, 'object:', typeof object);
  ...
  async.map(files, fs.readFile, function(err, sources) {
    ...
    for (var nickname in transformersByNickname) {
      ...
      for (var i = 0; i < dependencies.length; i++) {
        ...
        if (dependencyPath.indexOf('/') == 0) {
        ...
        } else if (components[0] == '__module__') {
          ...
          if (parent.order() >= child.order()) {
            ...
            try {
              ...
              var err = new Error('Cyclical inner-module dependency: ' +
                  dependencyNickname + ' in script: ' + this.files_[nickname]);
              callback(err);
              return;
            }
          }
    ...
    async.map(dependenciesToLoad, loadModule, function(err, results) {
      ...
      var loadOrderNicknames = nicknameDependencyTree.descendants().map(function(child) {
        return child.name;
      });
