...
ScriptResource.prototype.aliasForDependencyPath = function(dependencyPath) {
  return dependencyPath.replace(/[^a-zA-Z0-9_]/g, '_') + '_' + (+new Date());
};
...
ScriptResource.prototype.bodyContentType = function() {
  var depsInWindow = this.getAbsoluteDependencyPaths().map(function(globalDep) {
    return 'skit.objects[' + JSON.stringify(globalDep) + ']';
  });
  var body = 'skit.objects[' + JSON.stringify(this.resourcePath) + '] = ' +
                 '(' + this.getFunctionString() + ')(' + depsInWindow.join(',') + ');';
