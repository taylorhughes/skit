...
var Controller = util.createClass({
   ...
   * that should occur before render time. Only called once.
   *
  ...
  /**
   * Called on the server and client after preload has completed.
   */
  __load__: function() {
  ...
  /**
   * Transform the body from whatever it is (probably HTML) to HTML.
   *
   * @param {string} body The full body (result of __body__() calls) from this
   *     page.
   */
  ...
  recursiveUnload: function() {
    var current = this.constructor;
    while (current) {
      if (current.prototype.hasOwnProperty('__unload__')) {
        current.prototype.__unload__.call(this);
      }
      current = current.__parent__;
    }
  },
