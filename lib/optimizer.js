...
function optimizeServer(server, optimizedPackagePath, opt_options) {
  ...
  console.log('Building all publicly accessible files...');

  var publicRoot = server.getPublicRoot();
  var routeModules = loader.loadOptimizableModulesInRoot(publicRoot);

  var excludeModules = options.excludeModules || [];

  routeModules.forEach(function(controllerModule) {
    if (excludeModules.indexOf(controllerModule.modulePath) != -1) {
      return;
    }
